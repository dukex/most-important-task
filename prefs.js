...
class RulesList extends GObject.Object {
  ...
  constructor() {
      ...
      this.#changedId =
          this.#settings.connect(`changed::${"todoist-search-rules"}`, () => {
...
class RulesSettingsWidget extends Adw.PreferencesGroup {
  static {
      ...
      this.install_action('rules.add', null,
          self => self._rules.append());
      this.install_action('rules.remove', 's',
          (self, name, param) => self._rules.remove(param.unpack()));
      this.install_action('rules.rename', '(ss)',
          (self, name, param) => self._rules.rename(...param.deepUnpack()));
  }
...
function fillPreferencesWindow(window) {
  ...
  const todoistSearchLabel = new Gtk.Label({
    label: "Todoist API Search:",
    halign: Gtk.Align.START,
    visible: true,
  });
  group.add(todoistSearchLabel);

  const todoistSearchEntry = new Gtk.Entry({
    visible: true,
    text: settings.get_string("todoist-search"),
  });

  group.add(todoistSearchEntry);
  settings.bind(
    "todoist-search",
    todoistSearchEntry,
    "text",
